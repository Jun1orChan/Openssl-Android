...
char *DESUtil::des_cbc_enc(unsigned char *content, unsigned char *key, unsigned char *iv) {
    ...
    unsigned char *output = (unsigned char *) calloc(sizeof(unsigned char *), len);
    //IV
...
char *DESUtil::des_cbc_dec(unsigned char *content, unsigned char *key, unsigned char *iv) {
    ...
    for (int i = 0; i < len / 8; i++) {
        memcpy(inputText, cipherData + i * 8, 8);
        DES_ncbc_encrypt(inputText, outputText, 8, &key_schedule, ivec, DES_DECRYPT);
        memcpy(tmp, outputText, 8);
        for (int j = 0; j < 8; j++) {
            output[8 * i + j] = tmp[j];
        }
